...
public class TaskDAO {
	...
	public Optional<PagedResult<Task>> getTasksCreatedByManager(int managerId, Integer empId, String status, LocalDate fromDate,
		...
		sql.append("select t.*, COUNT(*) OVER() AS total_count" + "from tasks t" + "where t.created_by = ?");
		...
		if (status != null && !status.isBlank() && !status.equalsIgnoreCase("all")) {
			sql.append(" and t.status ilike ? ");
		}

		if (fromDate != null && toDate != null) {
			sql.append("and t.created_at::date between ? and ? ");
		}

		sql.append(" order by t.deadline ");
		...
		try (Connection con = DataSourceFactory.getConnectionInstance(); PreparedStatement pstmt = con.prepareStatement(sql.toString())) {
			...
			while (rs.next()) {
				tasks.add(mapToTask(rs));
				totalCount = rs.getInt("total_count"); // same value for every row
			}

			return Optional.of(new PagedResult<>(tasks, totalCount));

		} catch (SQLException e) {
			logger.error("DB error in getTasksCreatedByManager(managerId={}, empId={}, status={}, from={}, to={}, page={}, size={})",
							managerId, empId, status, fromDate, toDate, page, size, e);
			return Optional.empty();
		}
	}
